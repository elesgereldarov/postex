    ...
    <script>
        ...
        $(document).scannerDetection({
            ...
            onComplete: function(barcode, qty){ 
                num++;
                //var barcode = $('#barcodess').val(); //Math.random().toString(36).replace(/[^a-z]+/g, '').substr(0, 6);
                var obj = JSON.parse(jsonStr);
                obj['barcodes'].push({"barcode_id": num,"barcode_name": barcode});
                jsonStr = JSON.stringify(obj);   
                $('input[name=barcodes]').val(jsonStr);        
                $('.list-group').append('<li class="list-group-item delete_barcode" data-barcode_id="'+num+'"> <span class="icon voyager-x"></span> '+num+'. ' +barcode+ '</li>');
                $('#barcodess').val('');
        ...
        $(document).on('click', '.delete_barcode', function() {
            jsonStr = '{"barcodes": []}';
            var id = $(this).attr('data-barcode_id');
            $(this).remove();
            //var a = $('#barcodess').attr('data-barcode'); 
            var a = $('input[name=barcodes]').val();
            var objj = JSON.parse(a);
            var index = objj['barcodes'].indexOf(id);
            objj['barcodes'].splice(index, 1);
            a = JSON.stringify(objj);
            //$('#barcodess').attr('data-barcode',a); 
            $('input[name=barcodes]').val(a);   
            jsonStr = a;
        });
